...
- **Seeding is decided by the selected client.** A download is only removed once it has met qBittorrent's own ratio or seed-time limit. For Transmission, Maintainerr defers to Transmission whenever that torrent or the server has a ratio or idle-time rule, and only falls back when neither does. In that fallback case, Maintainerr waits until the download has both reached the **Fallback seeding ratio** and kept seeding for at least 23 hours.
