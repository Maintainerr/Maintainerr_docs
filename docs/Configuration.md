...
| Setting                | Description                                                                                                                                                                                                                                                                                                                              |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Client                 | Choose `qBittorrent` or `Transmission`. Nothing is selected until you choose. An install that already had a download client when it upgraded keeps `qBittorrent`, the only client that existed before. Changing the client clears the URL field because qBittorrent uses a WebUI base URL while Transmission uses the full RPC endpoint. |
...
- **Seeding is decided by the selected client.** A download is only removed once the selected client says its own seeding goal is met. If the selected client enforces no seeding goal at all, Maintainerr waits until the download has both reached the **Fallback seeding ratio** and kept seeding for at least 23 hours.
...
- For the supported torrent clients, if **Delete downloaded data** is on and another torrent shares the same downloaded files, Maintainerr removes only the torrent entry and keeps the data so the other torrent keeps working.
