...
- `Episode file rank by air date (newest = 1)` (number, episode rules only)
  ...
  match them.
- `Season file rank by air date (newest = 1)` (number, season rules only) is
...
- Rule 1: `Episode file rank by air date (newest = 1)` `bigger` than `5`
...
- Rule 1: `Season file rank by air date (newest = 1)` `bigger` than `2`
- Rule 2 (AND): `Series ID` `equals` the show's id
...
- Some servers store daily episodes with only an air date and no episode
  ...
  `Episode file rank by air date (newest = 1)` still resolves. When two
