...
- `Episode position by air date (1 = newest downloaded)` (number, episode rules only)
  ...
  match them.
- `Season position by air date (1 = newest downloaded)` (number, season rules only) is
...
- Rule 1: `Episode position by air date (1 = newest downloaded)` `bigger` than `5`
...
- Rule 1: `Season position by air date (1 = newest downloaded)` `bigger` than `2`
- Rule 2 (AND): `Series ID` `equals` the show's id
...
- Some servers store daily episodes with only an air date and no episode
  ...
  `Episode position by air date (1 = newest downloaded)` still resolves. When two
