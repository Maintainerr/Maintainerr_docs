...
Three Sonarr properties let you build a rolling window that keeps only the
newest downloaded episodes of a show and cleans up the rest.
...
during a run, affected episodes are skipped rather than treated as matches, so
