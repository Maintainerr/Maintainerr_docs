...
Rules that depend on watch history, such as `Plex.lastViewedAt`, `Jellyfin.lastViewedAt`, and related watch-state properties, treat watch-history lookup failures as an unknown value. Maintainerr does not interpret a failed lookup as "never watched".
...
After every rule run, items present in the shared media-server collection that aren't claimed by any of the linked rule groups are imported into each linked rule group as manual rows so Maintainerr doesn't strip them out. This import is not filtered by media type — an item added to the shared collection by one rule group can show up in another linked rule group's tracked count, including rule groups whose media type doesn't match.
...
To drain items from a shared custom collection, re-enable the rule group that originally added them, change its criteria so it matches nothing, and run it — the disable warning above applies.
...
A section is a group of rules. What happens to the result of a section depends on the choice of _Operator_ in the section's first rule.
