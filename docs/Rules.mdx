...
Some Tautulli, Tracearr, and Streamystats properties are scoped to one selected user: `Times viewed by user`, `Watch time by user (minutes)`, and `Last view date by user`.

- When you choose one of these properties, Maintainerr also asks you to pick a media-server username on that rule.
- If the saved username can no longer be resolved during a later rule run, Maintainerr treats the value as unknown for that evaluation instead of reading it as `0` or "never watched".
